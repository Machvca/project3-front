...
export default function NewRecipe() {
  ...
  const handleIngredientId = (ing) => {
    ing.map((e) => {
      return axios
        ...
        .then((response) => {
          setProductsId([...productsId, response.data[0]._id]);
        })
        .catch((error) => console.log(error));
    });
  };

  const createNewProduct = (recipe) => {
    axios
      .post(`${REACT_APP_API_URI}/api/recipe/create/`, recipe, {
  ...
  useEffect(() => {
    if (productReady && !fieldEmpty) {
      createNewProduct(newRecipe);
    }
  }, [newProduct, productReady]);
  ...
  return (
    <div className="productPage">
      <div className="productBody">
        ...
        {formStep === 3 && <NewRecipeStep3 step={step} setStep={setStep} />}
          ...
          {fieldEmpty ? (
            <Collapse in={open}>
              <Fade in={fieldEmpty}>
                ...
                >
                  There are fields empty — check it out!
                </Alert>
              </Fade>
            </Collapse>
          ) : (
